...
auto SSE::p(int pr0) -> int {
  ...
  switch( stats->blockType ) {
    ...
    case IMAGE32: {
      ...
      pr0 = (pr0 + pr1 + pr2 + pr3 + 2) >> 2U;
      pr1 = Image.Color.APM1s[0].p(pr, finalize64(hash(c0, stats->Image.pixels.W, (c4 & 0xffU) - stats->Image.pixels.Wp1, stats->Image.plane), 16));
      pr2 = Image.Color.APM1s[1].p(pr, finalize64(hash(c0, stats->Image.pixels.N, (c4 & 0xffU) - stats->Image.pixels.Np1, stats->Image.plane), 16));
