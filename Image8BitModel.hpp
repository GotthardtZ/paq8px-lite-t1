    ...
    void mix(Mixer &m) {
      ...
      if( bpos == 0 ) {
        ...
        if( x || !isPNG ) {
          ...
          NWWWW = buffer(w + 4), NWWW = buffer(w + 3), NWW = buffer(w + 2), NW = buffer(w + 1), N = buffer(
                  w), NE = buffer(w - 1), NEE = buffer(w - 2), NEEE = buffer(w - 3), NEEEE = buffer(w - 4);
          NNWWW = buffer(w * 2 + 3), NNWW = buffer(w * 2 + 2), NNW = buffer(w * 2 + 1), NN = buffer(
                  w * 2), NNE = buffer(w * 2 - 1), NNEE = buffer(w * 2 - 2), NNEEE = buffer(w * 2 - 3);
          NNNWW = buffer(w * 3 + 2), NNNW = buffer(w * 3 + 1), NNN = buffer(w * 3), NNNE = buffer(
                  w * 3 - 1), NNNEE = buffer(w * 3 - 2);
          ...
          if( !gray ) {
            ...
            MapCtxs[j++] = clip((buffer(w * 5) - 6 * buffer(w * 4) + 15 * NNN - 20 * NN + 15 * N +
                                 clamp4(W * 2 - NWW, W, NW, N, NN)) / 6);
            MapCtxs[j++] = clip((-3 * WW + 8 * W +
            ...
            MapCtxs[j++] = clip(
                    ((NW + NWW) / 2) * 3 - buffer(w * 2 + 3) * 3 + (buffer(w * 3 + 4) + buffer(w * 3 + 5)) / 2);
            ...
            MapCtxs[j++] = clip((-buffer(w * 4) + 5 * NNN - 10 * NN + 10 * N +
                                 clip(W * 4 - NWW * 6 + buffer(w * 2 + 3) * 4 - buffer(w * 3 + 4))) / 5);
            ...
            cm.set(hash(++i, (W + clamp4(NE * 3 - NNE * 3 + NNNE, W, N, NE, NEE)) / 2, px,
      ...
      if( x || !isPNG ) {
        ...
        m.set(c0, 256);
        m.set(((abs((int) (W - N)) > 4) << 9) | ((abs((int) (N - NE)) > 4) << 8) | ((abs((int) (W - NW)) > 4) << 7) |
              ((W > N) << 6) | ((N > NE) << 5) | ((W > NW) << 4) | ((W > WW) << 3) | ((N > NN) << 2) |
              ((NW > NNWW) << 1) | (NE > NNEE), 1024);
