...
void XMLModel::detectContent(XMLContent *content) {
  ...
  if((shared->c4 & 0xFFFFFAU) == 0xC2B042 && (shared->c4 & 0xffU) != 0x47 &&
     (((shared->c4 >> 24U) >= 0x30 && (shared->c4 >> 24U) <= 0x39) || ((shared->c4 >> 24U) == 0x20 && (buf(5) >= 0x30 && buf(5) <= 0x39))))
...
void XMLModel::update() {
  ...
  pState = state;
  if((shared->c1 == TAB || shared->c1 == SPACE) && (shared->c1 == (uint8_t) (shared->c4 >> 8U) || !whiteSpaceRun)) {
    whiteSpaceRun++;
    indentTab = (shared->c1 == TAB);
  } else {
  ...
  if( shared->c1 == NEW_LINE )
    lineEnding = 1 + ((uint8_t) (shared->c4 >> 8U) == CARRIAGE_RETURN);
  ...
  cm.set(hash(++i, state * 2 + (*tag).endTag, (*tag).name, (*tag).content.type, shared->c4 & 0xE0FFU));
}
