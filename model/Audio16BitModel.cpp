...
void Audio16BitModel::mix(Mixer &m) {
  if( shared->bitPosition == 0 && stats->blPos != 0 ) {
    ch = (stereo) ? (stats->blPos & 2U) >> 1U : 0;
    lsb = (stats->blPos & 1U) ^ (wMode < 4);
    if((stats->blPos & 1U) == 0 ) {
  ...
  const auto b = short(
          (wMode < 4) ? (lsb) ? uint8_t(shared->c0 << (8 - shared->bitPosition)) : (shared->c0 << (16 - shared->bitPosition)) | shared->c1
                      : (lsb) ? (shared->c1 << 8U) | uint8_t(shared->c0 << (8 - shared->bitPosition)) : shared->c0
  ...
  for( int i = 0; i < nSSM; i++ ) {
    ...
    sMap1B[i][1].set((lsb << 16U) | (shared->bitPosition << 13U) | (ctx0 >> ((!lsb) + (3U << shift))));
    sMap1B[i][2].set((lsb << 16U) | (shared->bitPosition << 13U) | (ctx0 >> ((!lsb) * 2 + (3U << shift))));
    sMap1B[i][3].set((lsb << 16U) | (shared->bitPosition << 13U) | (ctx1 >> ((!lsb) + (3U << shift))));
    sMap1B[i][0].mix(m);
    sMap1B[i][1].mix(m);
    sMap1B[i][2].mix(m);
    sMap1B[i][3].mix(m);
