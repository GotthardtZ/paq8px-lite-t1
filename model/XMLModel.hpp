...
#if (CACHE_SIZE & (CACHE_SIZE - 1)) || (CACHE_SIZE < 8)
...
public:
    ...
    void update() {
      ...
      INJECT_SHARED_c4
      INJECT_SHARED_c8
      XMLTag *pTag = &cache.tags[(cache.Index - 1) & (CACHE_SIZE - 1)], *tag = &cache.tags[cache.Index & (CACHE_SIZE - 1)];
      ...
      switch( state ) {
        ...
        case ReadTagName: {
          ...
          int i = 1;
          do {
            pTag = &cache.tags[(cache.Index - i) & (CACHE_SIZE - 1)];
            i += 1 + ((*pTag).endTag && cache.tags[(cache.Index - i - 1) & (CACHE_SIZE - 1)].name == (*pTag).name);
          } while( i < CACHE_SIZE && ((*pTag).endTag || (*pTag).empty));
        ...
        case ReadContent: {
          if( c1 == 0x3C ) {
            state = ReadTagName;
            cache.Index++;
            memset(&cache.tags[cache.Index & (CACHE_SIZE - 1)], 0, sizeof(XMLTag));
            cache.tags[cache.Index & (CACHE_SIZE - 1)].Level = (*tag).Level + 1;
          } else {
            (*content).length++;
      ...
      pTag = &cache.tags[(cache.Index - 1) & (CACHE_SIZE - 1)];
      uint64_t i = 64;
